...
# You find the file here: https://gofile.io/d/2434FN
...
do
  ...
  $startflink
  START=$(date +%s)
  $flink run -c Q6_ITERPattern_I1 $jar --input $data_path2 --output $output_path --times 7 --tput 60000 --vel 172
  END=$(date +%s)
  DIFF=$((END-START))
  ...
  $startflink
  START=$(date +%s)
  $flink run -c Q6_ITERQuery_I1T7 $jar --input $data_path2 --output $output_path --times 7 --tput 105000 --vel 172
  END=$(date +%s)
  DIFF=$((END-START))
  ...
  $startflink
  START=$(date +%s)
  $flink run -c Q6_ITERPattern_I1 $jar --input $data_path2 --output $output_path --times 9 --tput 45000 --vel 168
  END=$(date +%s)
  DIFF=$((END-START))
  ...
  $startflink
  START=$(date +%s)
  $flink run -c Q6_ITERQuery_I1T9 $jar --input $data_path2 --output $output_path --times 9 --tput 105000 --vel 168
  END=$(date +%s)
  DIFF=$((END-START))
