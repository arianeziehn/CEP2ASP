...
public class Q7_ITERQuery_I2 {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<Tuple2<KeyedDataPointGeneral, Integer>> result = aggStream.flatMap(new FlatMapFunction<Tuple3<KeyedDataPointGeneral, Integer, Integer>, Tuple2<KeyedDataPointGeneral, Integer>>() {
            final HashSet<Tuple2<KeyedDataPointGeneral, Integer>> set = new HashSet<Tuple2<KeyedDataPointGeneral, Integer>>(1000);

            // catch duplicates
            @Override
            public void flatMap(Tuple3<KeyedDataPointGeneral, Integer, Integer> t, Collector<Tuple2<KeyedDataPointGeneral, Integer>> collector) throws Exception {

                if (t.f2 >= iter) { // here we are approximate, we report that we detected at least the number of specified events, thus, if > iter we know that the window contains multiple sequences
                    Tuple2<KeyedDataPointGeneral, Integer> result = new Tuple2<>(t.f0, t.f2);
